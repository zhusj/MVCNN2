...
%     out(1,1,:,:) = tmp1;
    ...
    %%% ave pooling
        tmp = mean(out,4);
        for i = 1:12
%             dist_vector{j}(i) = sum(abs(tmp-out(:,:,:,i)));
            dist_vector{j}(i) = norm(squeeze(tmp-out(:,:,:,i)));
        end
        dist_vector{j} = dist_vector{j}/sum(dist_vector{j});
...
%       tmp = max(out,[],4);
    ...
    if isfield(l, 'weights')
     ...
     res(30).xx(1,1,1:dim3,j) = single(gather(tmp));    
end

% if opts.weighted
    res(30).dist_v = dist_vector;
% end
res(30).xx = gpuArray(res(30).xx);
res(30).out = single(res(30).xx);