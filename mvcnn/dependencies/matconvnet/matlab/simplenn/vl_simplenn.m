...
%                                'weightDecay', [opts.weightDecay 0]) ;
    net_n.layers{end+1} = struct('type', 'conv', 'name', 'fc_p1', ...
                               'weights', {{0.001/opts.scale * randn(13, 13, 512, numel(net.layers{end}.class), 'single'), ...
                               initBias*ones(1,numel(net.layers{end}.class),'single')}}, ...
                               'stride', 1, ...
                               'pad', 0, ...
                               'learningRate', [10 20], ...
                               'weightDecay', [opts.weightDecay 0]) ;
...
%     net_n.layers{end}.class = poses;
    ...
    res_n = struct(...
    'x', cell(1,numOfLayers+1), ...
    'dzdx', cell(1,numOfLayers+1), ...
    'dzdw', cell(1,numOfLayers+1), ...
    'aux', cell(1,numOfLayers+1), ...
    'time', num2cell(zeros(1,numOfLayers+1)), ...
    'backwardTime', num2cell(zeros(1,numOfLayers+1)), ...
    'pose', cell(1,numOfLayers+1)) ;
end
%%%%%%%%%%%%%%%%
...
%                     cudnn{:}) ;
              if i == 23
                 ...
                 res(i+1).x = vl_nnconv(res(i).x, l.filters, l.biases, ...
                          'pad', l.pad, 'stride', l.stride, ...
                          cudnn{:}) ;
                 if i == 16 && opts.concatenateFC6 && ~opts.test
                     [sz1,sz2,sz3,sz4] = size(res(i+1).x);
                     res(i+1).x = reshape(res(17).x,sz1,sz2,sz3*opts.views,sz4/opts.views);
                 end
          end
            ...
            case 'conv'
              if isfield(l, 'weights')
                res_n(i+1).x = vl_nnconv(res_n(i).x, l.weights{1}, l.weights{2}, ...
                  ...
                  res_n(i+1).x = vl_nnconv(res_n(i).x, l.filters, l.biases, ...
                               'pad', l.pad, 'stride', l.stride, ...
                               cudnn{:}) ;
              end
            case 'softmaxloss'
...
%             end
            ...
            % Legacy code: will go
            if opts.addSupervision && i == 12
                res_13_dzdx_sum = sum(sum(sum(sum(abs(res(13).dzdx)))));
...
%                 [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
%                     vl_nnconv(res(i).x, l.filters, l.biases, ...
%                               res(13).dzdx + res_n(1).dzdx/pose_dzdx_sum*res_13_dzdx_sum, ...
%                               'pad', l.pad, 'stride', l.stride, ...
%                               cudnn{:}) ;
                [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
                    vl_nnconv(res(i).x, l.filters, l.biases, ...
                              res(13).dzdx + res_n(1).dzdx, ...
                              'pad', l.pad, 'stride', l.stride, ...
                              cudnn{:}) ;
       %%%%%%%%%% Decay
%                 [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
...
%                               cudnn{:}) ;
            ...
            if i == 16 && opts.concatenateFC6 && ~opts.test
                ...
                res(i+1).dzdx=reshape(res(i+1).dzdx,sz1,sz2,sz3/opts.views,sz4*opts.views);
            end
%             [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
%                 vl_nnconv(res(i).x, l.filters, l.biases, ...
%                           res(i+1).dzdx, ...
...
% %                 [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
% %                     vl_nnconv(res(i).x, l.filters, l.biases, ...
% %                               res(12).dzdx + res_n(1).dzdx/pose_dzdx_sum*res_12_dzdx_sum, ...
% %                               'pad', l.pad, 'stride', l.stride, ...
% %                               cudnn{:}) ;
%                res(i).dzdx = vl_nnrelu(res(i+1).x, res(i+1).dzdx + res_n(1).dzdx/pose_dzdx_sum*res_13_dzdx_sum, leak{:}) ;
%            else
