function [c,u,s] = learn_HOSVD(GCF)
...
if nargin<1
    filename = 'AVLetter_D1-Letters_D2-reptns(2-3)_D3-Subjects_D4-5-SequenceFeatures';
...
nGCF = reshape(nGCF,size(nGCF,1)*size(nGCF,2),size(nGCF,3),size(nGCF,4));%another option is to merge repeatation with letter to increase accuracy of letters classification
...
rdim = size(nGCF)%[10, 3, 11, 220] % 10           3       45056 ( 11 x 4096)
%rdim(end) = rdim(end)/100;

tic
%[c u s ] = HOSVDDR2(GCF,rdim,5);
[c u s ] = HOSVDDR2(nGCF,rdim);
...
save('AVLetter_Learn_HOSVD_52-10-(2-3)','c','u','s','rdim');
