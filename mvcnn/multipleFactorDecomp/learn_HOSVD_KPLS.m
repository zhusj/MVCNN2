function storeFile = learn_HOSVD_KPLS(run_stamp,filename,plot_flg)
...
ltr_labels = eye(train_seq_dim(1));%[1:26,1:26]';
labels{1} = repmat(ltr_labels,train_seq_dim(2),1);
labels{2} = eye(train_seq_dim(3));
labels{3} = [];%ones(size(GCF,3),1);
...
rdim(1) = 40;
rdim(2) = 10;
%rdim(1) = train_seq_dim(1);
%rdim(2) = train_seq_dim(3);
%rdim(end) = rdim(end)/100;
%rdim(1) = rdim(1)/2;
tic
%[c u s ] = HOSVDDR2(GCF,rdim,5);
[c u ] = HOSVD_KPLS(GCF,rdim,labels,5);
