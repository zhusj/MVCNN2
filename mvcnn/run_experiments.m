...
ex(end+1).trainOpts = struct(...
    ...
    'multiview', false, ...
    'viewpoolLoc', 'conv5', ...
    'learningRate', [1e-4*ones(1,3) 3e-5*ones(1,3) 1e-5*ones(1,3) 1e-6*ones(1,3)], ...%logspace(-4,-6,10), ...%[1e-4*ones(1,3) 3e-5*ones(1,3) 1e-5*ones(1,3) 1e-6*ones(1,3)], ...
    'momentum', 0.5, ...
    'includeVal', false, ...
    'useUprightAssumption', false); 
ex(end).featOpts = struct(...
...
%         pose_addsup_12_views_10_epochs_add_dropoot_fc6_fc7_feature_fusion_fc6+fc7
        prefix = 'vgg_no_view_pooling_ori_LR_no_val';
%         prefix = sprintf('BS%d_AUG%s', trainOpts.batchSize, trainOpts.aug);
%         prefix = 'vgg_12_projectors_fc7';
