...
logPath = fullfile('log','eval_12branch.txt'); 
skipTrain = false; 
skipEval = false; 
ex = struct([]);
...
ex(end+1).trainOpts = struct(...
    ...
    'multiview', true, ...
    'viewpoolLoc', 'conv5', ...
    'learningRate', 2*logspace(-3,-6,10), ...%logspace(-3,-4,15), ...%[1e-4*ones(1,3) 3e-5*ones(1,3) 1e-5*ones(1,3) 1e-6*ones(1,3)], ...
    'momentum', 0.5, ...
    'includeVal', false, ...
    'useUprightAssumption', false); 
ex(end).featOpts = struct(...
...
%         prefix = sprintf('BS%d_AUG%s', trainOpts.batchSize, trainOpts.aug);
        prefix = 'vgg_with_add_12_branch_after_fc6_ave_pooling_LR2log-3-6';
