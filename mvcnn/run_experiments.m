...
% ex(end+1).trainOpts = struct(...
%     'baseModel', 'imagenet-vgg-m-finetuned-ModelNet40v1-BS60_AUGnone', ...'imagenet-vgg-m-finetuned-ModelNet40v1-BS60_AUGnone', ... 
%     'dataset', 'ModelNet40v1', ...
%     'batchSize', 60, ... 
%     'aug', 'none', ...
%     'numEpochs', 5, ...
...
%     'multiview', true, ...
%     'viewpoolLoc', 'conv5', ...
%     'learningRate', [3e-4*ones(1,3) 6e-5*ones(1,3) 1e-5*ones(1,3) 5e-6*ones(1,3)], ... %4*logspace(-4,-6,10), ...%2*logspace(-4,-6,10), ...%[1e-4*ones(1,3) 3e-5*ones(1,3) 1e-5*ones(1,3) 1e-6*ones(1,3)], ...
%     'momentum', 0.5, ...
%     'includeVal', false, ...
%     'useUprightAssumption', false, ...
%     'projectorSize', 1024); 
...
% 
ex(end+1).trainOpts = struct(...
    'baseModel', 'imagenet-caffe-alex', ...'imagenet-vgg-m-finetuned-ModelNet40v1-BS60_AUGnone', ... 
    'dataset', 'ModelNet40v1', ...
    'batchSize', 60, ... 
    'aug', 'none', ...
    'numEpochs', 10, ...
    'gpuMode', true, ... % 'multiview', true, ...
    'viewpoolLoc', 'conv5', ...
    'learningRate', [3e-4*ones(1,3) 6e-5*ones(1,3) 1e-5*ones(1,3) 5e-6*ones(1,3)],...%[3e-4*ones(1,3),6e-5], ... %[3e-4*ones(1,3) 6e-5*ones(1,3) 1e-5*ones(1,3) 5e-6*ones(1,3)], ... %4*logspace(-4,-6,10), ...%2*logspace(-4,-6,10), ...%[1e-4*ones(1,3) 3e-5*ones(1,3) 1e-5*ones(1,3) 1e-6*ones(1,3)], ...
    'momentum', 0.5, ...
    'includeVal', false, ...
    'useUprightAssumption', false, ...
    'projectorSize', 2048); 
...
%           prefix = 'vgg_with_max_view_pooling_addSup_conv5_Von_Mises_kernel_l2Normalization_15_epoch_log_-3_LR';
        prefix = sprintf('BS%d_AUG%s_addDropout', trainOpts.batchSize, trainOpts.aug);
%         prefix = 'vgg_with_add_12_branch_after_fc6_ave_pooling';
        
%         prefix = 'fine_tuned_vgg_add_12_branch_2048_after_fc6_LRw4_ave_pooling_leaky0.01_8epochs_rmfc7';
%         prefix = sprintf('fine_tunedvgg_add_12_branch_%d_after_fc6_LRw4_ave_pooling_normWeightedbackprop_leaky0.01_%depochs_rmfc7', trainOpts.projectorSize, trainOpts.numEpochs);
