...
% Add branch layers
if opts.addBranch, 
    net.layers{end-1}.name = 'fc8_class';
    net.layers{end}.name = 'softmax_class';
    net.layers{end-1}.branch = 'class';
    net.layers{end}.branch = 'class';
    
    net.layers = horzcat(net.layers, ...
                            net.layers(end-1:end));
                        
    net.layers{end-1}.biases = zeros(1, 12, 'single');
    net.layers{end-1}.name = 'fc8_pose';
    net.layers{end}.name = 'softmax_pose';
    net.layers{end-1}.branch = 'pose';
    net.layers{end}.branch = 'pose';
%     opts.scale = 1;
%     opts.weightDecay = 1 ;
% %     net = add_block(net, opts, 8, 1, 1, 4096, 12, 1, 0, 0) ;
% %     net.layers(end) = [] ;
% %     net.layers{end+1} = struct('type', 'softmaxloss', 'name', 'loss') ;
end

% add supervision  after conv5, add fc and softmax layers
...
trainOpts.addSupervision = opts.addSupervision;
...
% -------------------------------------------------------------------------
function [im,labels,poses] = getBatch(imdb, batch, opts, varargin)
% -------------------------------------------------------------------------
...
% branch
names = imdb.images.name(batch(idxs)) ;
poses = zeros(size(names));
angle_interval = 1;%30;
for i = 1:length(names)
    poses(i) = str2double(names{i}(end-5:end-4)) * angle_interval;
end
...
net = add_block(net, opts, 1, 3, 3, 49152, 500, 1, 0, init_bias); 
net.layers{end+1} = struct('type', 'dropout', 'name', 'dropout1', 'rate', 0.5);

net = add_block(net, opts, 2, 1, 1, 500, 500, 1, 0, init_bias); 
net.layers{end+1} = struct('type', 'dropout', 'name', 'dropout2', 'rate', 0.5);

net = add_block(net, opts, 3, 1, 1, 500, numClass, 1, 0, 0);
net.layers(end) = [];
